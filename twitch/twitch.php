...
use Akeeba\SocialLogin\Twitch\OAuth as TwitchOAuth;
use Akeeba\SocialLogin\Twitch\UserQuery;
...
/**
 * Akeeba Social Login plugin for Twitch integration
 */
class plgSocialloginTwitch extends AbstractPlugin
{
	/**
	...
	public function __construct($subject, array $config = array())
	{
		...
		JLoader::registerNamespace('Akeeba\\SocialLogin\\Twitch', __DIR__ . '/Twitch', false, false, 'psr4');
	...
	protected function getConnector()
	{
		if (is_null($this->connector))
		{
			...
			$this->connector = new TwitchOAuth($options, $httpClient, $app->input, $app);
			$this->connector->setScope('user_read');
		}
	 ...
	 */
	 protected function mapSocialProfileToUserData(array $socialProfile)
 	{
 		...
 		$userData->id       = isset($socialProfile['_id']) ? $socialProfile['_id'] : '';
 		$userData->email    = isset($socialProfile['email']) ? $socialProfile['email'] : '';
 		$userData->verified = isset($socialProfile['email_verified']) ? $socialProfile['email_verified'] : true;

 		return $userData;
 	}

	/**
	 * Processes the authentication callback from Twitch.
	 *
